...
class PersistentQuizRepository(
    ...
    override suspend fun getAll(): List<Quiz> {
        ...
        val schedulersConfigurationsByQuizId = database.schedulerConfigurationQueries
            ...
            .associate {
                it.quiz_id to QuizSchedulerConfiguration(
                    id = it.id,
                    selection = it.selection,
                    isEnabled = it.enabled == true
                )
    ...
    override suspend fun getById(id: String): Result<Quiz> {
        ...
        val schedulerConfiguration = database.schedulerConfigurationQueries
            ...
            .executeAsOneOrNull()
            ?.let { scheduler -> QuizSchedulerConfiguration(scheduler.id, scheduler.selection, scheduler.enabled == true) }
        val quiz = Quiz(
            id = quizEntity.id,
    ...
    private fun updateSchedulerConfiguration(
        quizId: String,
        schedulerConfiguration: QuizSchedulerConfiguration?
    ) {
    ...
    override suspend fun getAllScheduled(): Map<String, QuizSchedulerConfiguration> {
        return database.schedulerConfigurationQueries.getAllScheduled()
            .executeAsList()
            .associate {
                it.id to QuizSchedulerConfiguration(
                    id = it.id,
                    selection = it.selection,
                    isEnabled = it.enabled == true
                )
