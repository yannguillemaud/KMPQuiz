...
fun HomeScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            SectionHeader(title = "Overview", icon = Icons.Default.Info)
            ElevatedCard(modifier = Modifier.fillMaxWidth()) {
                Column {
                    ListItem(
                        headlineContent = { Text("Categories", fontWeight = FontWeight.SemiBold) },
                        supportingContent = {
                            if (homeState.totalCategoriesCount == 0) Text("No categories yet.")
            ...
            SectionHeader(title = "Available Sources", icon = Icons.Default.Download)
            LazyColumn {
                items(homeState.sources.toList(), key = { it.first }) { (id, fetcher) ->
                    ElevatedCard {
                        Column {
                            ListItem(
                                headlineContent = { Text(fetcher.name) },
                                trailingContent = {
                                    if (!fetcher.isDownloading) {
                                        FilledTonalIconButton(onClick = {
                                            viewModel.processIntent(DownloadIntent.Fetch(id))
                                        }) {
                                            Icon(
                                                Icons.Outlined.CloudDownload,
                                                contentDescription = "Download ${fetcher.name}"
                                            )
                                        }
