...
class CreateQuizUseCase(
    ...
    suspend fun updateBy(
        ...
        return try {
            val allQandas = getQandaUseCase.observeAll().first()
            val filteredQandas = allQandas.filter { predicate(it) }
            val existing = getQuizUseCase.getQuizById(title)
            val quizQandas = existing?.qandas ?: emptyList()
            val cron = existing?.quizCron
            val quizDraft = QuizDraft(title, quizQandas + filteredQandas, cron)
    ...
    suspend fun createBy(
        ...
        return try {
            ...
            val quizDraft = QuizDraft(title, filteredQandas, QuizCron)
            val result = quizRepository.insertQuiz(quizDraft)
            if (result.isSuccess) {
                logger.i { "Created quiz $title with ${filteredQandas.size} qandas" }
            }
