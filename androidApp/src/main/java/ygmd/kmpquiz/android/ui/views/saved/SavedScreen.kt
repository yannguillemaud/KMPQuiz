...
import ygmd.kmpquiz.viewModel.save.CategoryQandas
...
fun SavedScreen(
    ...
    val uiState by viewModel.savedState.collectAsState(SavedQandasUiState())
    ...
    BottomSheetScaffold(
        ...
        LazyColumn(
            ...
            items(uiState.identifiedQandas, { it.identifier }) { categoryQanda ->
                val category = categoryQanda.identifier
                Card(
                    modifier = Modifier.fillMaxWidth(),
                ) {
                    SavedCategoryCard(
                        categoryQanda = categoryQanda,
                        ...
                        onDeleteCategory = {
                            viewModel.processIntent(
                                PersistanceIntent.DeleteByIdentifier(categoryQanda.identifier)
                            )
                        },
                        onDeleteQanda = { id ->
                            viewModel.processIntent(
    ...
    Column(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = categoryQanda.identifier,
                    style = typography.titleMedium
                )
                Text(
                    text = "${categoryQanda.qandas.size} questions",
        ...
        if (isExpanded) {
            ...
            categoryQanda.qandas.forEach { qanda ->
                QandaItem(
                    qanda = qanda,
